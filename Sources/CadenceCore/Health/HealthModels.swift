...
/// One calendar day of Apple Health metrics. Everything is optional because
/// the user may have denied any individual category, and a day with no
/// data is still a valid (empty) day.
...
/// Aggregations over `[HealthDay]` for the dashboard and charts.
public enum HealthStats {
    ...
    /// Days in `range`, oldest first, zero-filled so charts show gaps.
...
/// Merges runs read from Apple Health into the local list.
public enum HealthImport {

    /// Adds imported runs the user hasn't seen before. A run whose
    /// `healthKitID` already exists locally is skipped — including if the
    /// user deleted it, so a deletion in Cadence isn't undone by the next
    /// import. Runs the app itself exported to Health (their `healthKitID`
    /// was assigned on export) are also skipped, so they don't come back as
    /// duplicates.
    ...
    /// Replaces cached days with newly fetched ones, keeping older cached
    /// days that weren't part of this fetch.
